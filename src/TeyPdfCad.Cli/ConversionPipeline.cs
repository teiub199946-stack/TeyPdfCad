...
public sealed class ConversionPipeline
{
    public async Task<ConversionResult> ConvertAsync(string inputPdfPath, string outputDwgPath, string reportPath, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                await WriteReportAsync(reportPath, complete: false, document.PageCount, 0, ["PDF has no usable vector entities; raster/scanned input is unsupported in this release."], cancellationToken);
            ...
            await WriteReportAsync(reportPath, complete, document.PageCount, pagesWithVectors, warnings, cancellationToken);
        ...
        {
            await WriteReportAsync(reportPath, complete: false, 0, 0, [exception.Message], cancellationToken);
            return new ConversionResult(ConversionOutcome.InvalidArgumentsOrIo, reportPath, null);
        }
    }

    private static Task WriteReportAsync(string path, bool complete, int pagesRead, int pagesProcessed, IReadOnlyList<string> warnings, CancellationToken cancellationToken)
