...
public static class DwgEntityFingerprint
{
    ...
    public static string ComputeDimensionStyle(Dimension dimension)
    {
        ...
        return Join(
            ...
            style.AlternateUnitDimensioning ? "1" : "0",
            style.AlternateDimensioningSuffix ?? string.Empty,
            style.AlternateUnitDecimalPlaces.ToString(CultureInfo.InvariantCulture),
            Number(style.AlternateUnitScaleFactor),
            style.GenerateTolerances ? "1" : "0",
            style.LimitsGeneration ? "1" : "0",
            Number(style.PlusTolerance),
            Number(style.MinusTolerance),
            style.TextInsideExtensions ? "1" : "0",
            style.PostFix ?? string.Empty,
            style.DecimalPlaces.ToString(CultureInfo.InvariantCulture),
            ((int)style.DecimalSeparator).ToString(CultureInfo.InvariantCulture),
