...
public sealed class DimensionProvenanceTests
{
    ...
    public void Carries_Source_Ids_Into_Reconstructed_Dimension()
    {
        ...
        scene.Lines.Add(new LinePrimitive(new Point2(-1, -1), new Point2(1, 1), SourceIds: ["A1"]));
        scene.Lines.Add(new LinePrimitive(new Point2(51, -1), new Point2(53, 1), SourceIds: ["A2"]));
        ...
        Assert.Equal(6, dimension.ProvenanceIds.Count);
        ...
        Assert.Contains("A1", dimension.ProvenanceIds);
        Assert.Contains("A2", dimension.ProvenanceIds);
