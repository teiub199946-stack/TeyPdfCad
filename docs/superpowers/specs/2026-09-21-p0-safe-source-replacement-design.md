...
SuppressionGate is the only component allowed to grant suppression. It receives the existing, modified Core SourceReplacementPlan and NativeReadBackVerification. SuppressionDecision remains page-local; when DWG/CLI aggregate decisions across a document they must promote every authorized SourceId to PageSourceRef(PageNumber, SourceId). A flat document-wide set of raw SourceId strings is forbidden.
